TOWER_ABLI_SPX = {
    {
        Name = "封印枷锁之一",
        Ubertip = "中阶兵塔，通常能得到解锁",
        Art = "war3mapImported\\icon_pas_Slow_Grey.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1
    },
    {
        Name = "封印枷锁之二",
        Ubertip = "高阶兵塔，通常能得到解锁",
        Art = "war3mapImported\\icon_pas_Slow_Grey.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 2
    },
    {
        Name = "溅射攻击",
        Ubertip = "+溅射攻击",
        Art = "ReplaceableTextures\\PassiveButtons\\PASBTNFireRocks.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1
    },
    {
        Name = "月刃弹射",
        Ubertip = "+弹射攻击",
        Art = "ReplaceableTextures\\PassiveButtons\\PASBTNUpgradeMoonGlaive.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1
    },
    {
        Name = "投刃弹射",
        Ubertip = "+弹射攻击",
        Art = "ReplaceableTextures\\PassiveButtons\\PASBTNVorpalBlades.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1
    },
    {
        Name = "超远程火炮",
        Ubertip = "+300攻击距离",
        Art = "ReplaceableTextures\\PassiveButtons\\PASBTNHumanArtilleryUpOne.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1
    },
    {
        Name = "窥视镜",
        Ubertip = "+200攻击距离",
        Art = "ReplaceableTextures\\PassiveButtons\\PASBTNGyroCopterTrueSight.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1
    },
    {
        Name = "黑管火炮",
        Ubertip = "+100攻击距离",
        Art = "war3mapImported\\icon_pas_DwarvenLongRifle.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1
    },
    {
        Name = "突击匕首",
        Ubertip = "+{val#1}物理攻击",
        Art = "war3mapImported\\icon_pas_Ability_Rogue_BladeTwisting.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 2,
        Val = {125}
    },
    {
        Name = "月神强击",
        Ubertip = "+{val#1}物理攻击",
        Art = "ReplaceableTextures\\PassiveButtons\\PASBTNTrueShot.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 2,
        Val = {275}
    },
    {
        Name = "魔力之源",
        Ubertip = "+{val#1}魔法攻击",
        Art = "war3mapImported\\icon_pas_ManaRecharge.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 2,
        Val = {185}
    },
    {
        Name = "治疗",
        Ubertip = "+{val#1}生命恢复",
        Art = "war3mapImported\\icon_pas_Heal.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {0.04}
    },
    {
        Name = "丧心病狂",
        Ubertip = "攻击时有{val#1}%几率增加{val#2}%攻击速度，持续{val#3}秒",
        Art = "war3mapImported\\icon_pas_Berserker_Blood.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {15, 45, 2, "war3mapImported\\eff_Fire_Spear.mdl"}
    },
    {
        Name = "远古身躯",
        Ubertip = "+{val#1}力量",
        Art = "war3mapImported\\icon_pas_FireDragonElemental.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {36}
    },
    {
        Name = "重拳出击",
        Ubertip = "+{val#1}力量",
        Art = "war3mapImported\\icon_pas_AdvancedUnholyStrength.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {55}
    },
    {
        Name = "骑士精神",
        Ubertip = "+{val#1}力量",
        Art = "war3mapImported\\icon_pas_Holy_GreaterBlessingofKings.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {75}
    },
    {
        Name = "蛇皮",
        Ubertip = "+{val#1}敏捷",
        Art = "war3mapImported\\icon_pas_FireDragonElemental.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {30}
    },
    {
        Name = "六刃智慧",
        Ubertip = "+{val#1}智力|n+{val#2}魔法攻击",
        Art = "war3mapImported\\icon_pas_Gold_Six_Blades.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {36, 36}
    },
    {
        Name = "小牛粉碎",
        Ubertip = "+{val#1}%分裂",
        Art = "ReplaceableTextures\\PassiveButtons\\PASBTNSmash.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {20}
    },
    {
        Name = "分裂大刀",
        Ubertip = "+{val#1}%分裂",
        Art = "ReplaceableTextures\\PassiveButtons\\PASBTNCleavingAttack.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {25}
    },
    {
        Name = "猫头鹰怒视",
        Ubertip = "+{val#1}%命中",
        Art = "ReplaceableTextures\\PassiveButtons\\PASBTNScout.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {35}
    },
    {
        Name = "熊掌战意",
        Ubertip = "+{val#1}物理暴击几率|n+{val#2}物理暴击伤害",
        Art = "war3mapImported\\icon_pas_Regeneration.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {15, 10}
    },
    {
        Name = "刺客信条",
        Ubertip = "+{val#1}物理暴击几率|n+{val#2}物理暴击伤害",
        Art = "war3mapImported\\icon_pas_Spell_Holy_EyeforanEye.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {50, 20}
    },
    {
        Name = "法术研究",
        Ubertip = "+{val#1}魔法暴击几率|n+{val#2}魔法暴击伤害",
        Art = "war3mapImported\\icon_pas_WitchDoctorAdept.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {17, 10}
    },
    {
        Name = "野外龙种",
        Ubertip = "+龙攻击特性",
        Art = "war3mapImported\\icon_pas_Dragon_spray_flame.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {}
    },
    {
        Name = "树木之妖",
        Ubertip = "+木攻击特性",
        Art = "war3mapImported\\icon_pas_EatTree.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {}
    },
    {
        Name = "水之元素",
        Ubertip = "+水攻击特性",
        Art = "war3mapImported\\icon_pas_Elemental_Mote_Water01.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {}
    },
    {
        Name = "山岭之灵",
        Ubertip = "+土攻击特性",
        Art = "war3mapImported\\icon_pas_Taunt.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {}
    },
    {
        Name = "海洋之心",
        Ubertip = "+{val#1}%水攻击",
        Art = "war3mapImported\\icon_pas_Elemental_Mote_Water01.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {3}
    },
    {
        Name = "深海之心",
        Ubertip = "+{val#1}%水攻击",
        Art = "war3mapImported\\icon_pas_Elemental_Primal_Water.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {7}
    },
    {
        Name = "火球术",
        Ubertip = "+{val#1}%火攻击",
        Art = "war3mapImported\\icon_pas_Fire.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {3}
    },
    {
        Name = "暗夜之魂",
        Ubertip = "+{val#1}%暗攻击",
        Art = "war3mapImported\\icon_pas_Holy_ElunesGrace.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {5}
    },
    {
        Name = "海妖特质",
        Ubertip = "+{val#1}%水抗性",
        Art = "war3mapImported\\icon_pas_Spell_Frost_FrozenCore.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {30}
    },
    {
        Name = "净化",
        Ubertip = "有{val#1}%几率净化敌人，减少{val#2}点移动力{val#3}秒",
        Art = "war3mapImported\\icon_pas_Purge.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {30, 25, 3, "Abilities\\Spells\\Orc\\Purge\\PurgeBuffTarget.mdl"}
    },
    {
        Name = "灵魂净化",
        Ubertip = "有{val#1}%几率净化敌人，减少{val#2}点移动力{val#3}秒",
        Art = "war3mapImported\\icon_pas_Purge.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {30, 45, 4, "Abilities\\Spells\\Orc\\Purge\\PurgeBuffTarget.mdl"}
    },
    {
        Name = "蛊毒",
        Ubertip = "有{val#1}%几率中毒每秒扣血{val#2}点，持续{val#3}秒",
        Art = "ReplaceableTextures\\PassiveButtons\\PASBTNSlowPoison.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {50, 15, 4, "Abilities\\Weapons\\PoisonSting\\PoisonStingTarget.mdl"}
    },
    {
        Name = "蛛毒",
        Ubertip = "{val#1}%使中毒每秒扣血{val#2}点，持续{val#3}秒",
        Art = "war3mapImported\\icon_pas_Creature_Poison_06.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {100, 12, 6, "Abilities\\Weapons\\PoisonSting\\PoisonStingTarget.mdl"}
    },
    {
        Name = "腐尸毒",
        Ubertip = "有{val#1}%几率中毒每秒扣血{val#2}点，持续{val#3}秒",
        Art = "ReplaceableTextures\\PassiveButtons\\PASBTNPlagueCloud.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {60, 30, 4, "Abilities\\Weapons\\PoisonSting\\PoisonStingTarget.mdl"}
    },
    {
        Name = "燃油烧弹",
        Ubertip = "有{val#1}%几率灼烧每秒扣血{val#2}点，持续{val#3}秒",
        Art = "ReplaceableTextures\\PassiveButtons\\PASBTNLiquidFire.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {75, 40, 4, "Abilities\\Spells\\Orc\\LiquidFire\\Liquidfire.mdl"}
    },
    {
        Name = "悲鸣",
        Ubertip = "减少敌人{val#2}点减伤，持续{val#3}秒",
        Art = "ReplaceableTextures\\PassiveButtons\\PASBTNShadeTrueSight.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {100, 10, 30, "Abilities\\Spells\\Undead\\ReplenishHealth\\ReplenishHealthCaster.mdl"}
    },
    {
        Name = "震荡光弹",
        Ubertip = "{val#1}%几率眩晕{val#3}秒",
        Art = "war3mapImported\\icon_pas_Arcane_MassDispel.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {20, 0, 1.5, "Abilities\\Spells\\Other\\Silence\\SilenceAreaBirth.mdl"}
    },
    {
        Name = "暴力倾向",
        Ubertip = "{val#1}%几率眩晕{val#3}秒",
        Art = "war3mapImported\\icon_pas_Spell_Frost_IceFloes.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {30, 0, 0.75}
    },
    {
        Name = "魅惑",
        Ubertip = "{val#1}%几率眩晕{val#3}秒",
        Art = "war3mapImported\\icon_pas_Hunter_MasterTactitian.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {10, 0, 3, "Abilities\\Spells\\NightElf\\Barkskin\\BarkSkinTarget.mdl"}
    },
    {
        Name = "旋风",
        Ubertip = "{val#1}%几率刮起旋风造成{val#2}伤害",
        Art = "war3mapImported\\icon_pas_Cyclone.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {20, 50, 0.1, "Abilities\\Spells\\NightElf\\Cyclone\\CycloneTarget.mdl"}
    },
    {
        Name = "撸战棍",
        Ubertip = "{val#1}%几率造成{val#2}伤害并眩晕{val#3}秒",
        Art = "war3mapImported\\icon_pas_GrabTree.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {35, 60, 2.2, "war3mapImported\\eff_DustWindFasterExact.mdl"}
    },
    {
        Name = "诡异闪电",
        Ubertip = "{val#1}%几率击出一道红色闪电造成{val#2}点雷伤害",
        Art = "war3mapImported\\icon_pas_Glyph.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {20, 80}
    },
    {
        Name = "狂异闪电",
        Ubertip = "{val#1}%几率击出一道红色闪电造成{val#2}点雷伤害",
        Art = "war3mapImported\\icon_pas_Glyph.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {20, 100}
    },
    {
        Name = "风暴之锤",
        Ubertip = "{val#1}%几率击出一道闪电对{val#2}个敌人造成{val#3}点雷伤害",
        Art = "ReplaceableTextures\\PassiveButtons\\PASBTNStormHammer.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {20, 1, 60}
    },
    {
        Name = "离火之锤",
        Ubertip = "{val#1}%几率击出一道闪电对{val#2}个敌人造成{val#3}点雷伤害",
        Art = "war3mapImported\\icon_pas_Stimulate_lightning.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {20, 3, 75}
    },
    {
        Name = "皮糙",
        Ubertip = "+{val#1}生命",
        Art = "ReplaceableTextures\\PassiveButtons\\PASBTNDrum.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {200},
        ODK = true
    },
    {
        Name = "肉燥",
        Ubertip = "+{val#1}生命",
        Art = "ReplaceableTextures\\PassiveButtons\\PASBTNDrum.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {300},
        ODK = true
    },
    {
        Name = "全能守护",
        Ubertip = "+{val#1}生命",
        Art = "war3mapImported\\icon_pas_Holy_AvengersShield.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 2,
        Val = {600},
        ODK = true
    },
    {
        Name = "铁壁",
        Ubertip = "+{val#1}护甲",
        Art = "ReplaceableTextures\\PassiveButtons\\PASBTNThickFur.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {10},
        ODK = true
    },
    {
        Name = "捍卫守护",
        Ubertip = "+{val#1}护甲",
        Art = "war3mapImported\\icon_pas_Holy_AvengersShield.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 2,
        Val = {30},
        ODK = true
    },
    {
        Name = "塞壬之歌",
        Ubertip = "+{val#1}%魔抗",
        Art = "war3mapImported\\icon_pas_Holy_DivineHymn.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {25},
        ODK = true
    },
    {
        Name = "石像化",
        Ubertip = "+{val#1}点减伤",
        Art = "war3mapImported\\icon_pas_Rogue_DeadenedNerves.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {40},
        ODK = true
    },
    {
        Name = "黑暗舞步",
        Ubertip = "+{val#1}%回避",
        Art = "war3mapImported\\icon_pas_Shadow_Riki.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 1,
        Val = {15},
        ODK = true
    },
    {
        Name = "影子替身",
        Ubertip = "+{val#1}%回避",
        Art = "war3mapImported\\icon_pas_Undercurrent_white_light.blp",
        Buttonpos1 = 0,
        Buttonpos2 = 2,
        Val = {35},
        ODK = true
    }
}
